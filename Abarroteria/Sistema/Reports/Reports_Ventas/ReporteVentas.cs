    ...
    public partial class ReporteVentas : BaseContext
    {
        ...
        private void CargarTabla()
        {
            ...
            if(listadoFacturaEncabezado!= null)
            {
                ...
                {
                    //var detalle = _facturasRepository.GetDetallebyFactura(item.Id);
                    //listadoDetalleObtenido.Add((ListarDetalleFacturas)detalle);
                    ...
                    if (item.ProductoId != null)
                    {
                        ...
                        else
                        {
                            foreach (var detalletoUpdate in listaventastosee.Where(x => x.ProductoId == detalle.ProductoId))
                            {
                                ...
                                //detalletoUpdate.Utilidad += _productoRepository.Get(detalle.ProductoId).Utilidad;
                                detalletoUpdate.Subtotal += item.PrecioTotal;
                                //detalletoUpdate.CostoVenta += _productoRepository.Get(detalle.ProductoId).Coste;
                            }
                        }
                    }
                }
            ...
            //utilidadTotal = listaventastosee.Sum(x => x.Utilidad);
            //TotalVentas = listaventastosee.Sum(x => x.Subtotal);
