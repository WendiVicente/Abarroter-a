...
namespace Sistema.Forms.modulo_cotizacion
{
    public partial class NuevaCotizacion : BaseContext
    {
        ...
        //datagrid cotizacion 
        private int  productidColcotiz = 2, preciocolcotiz = 8, cantidadcolcotiz = 9, subtotalcolcotiz = 10;
        private int coloridColcotiz = 11, tallaidColcotiz = 12, tallaColorIdCotiz = 13, detalleidestilo = 14;
        ...
        public List<DetalleCotizacion> GetDatosDetalleCotiza()
        {
            ...
            foreach (DataGridViewRow item in dgvCotizacion.Rows)
            {
                ...
                ListarDetalleCotiz cot = (ListarDetalleCotiz)item.DataBoundItem;
                ...
                DetalleCotizacion detalleCotiz = new DetalleCotizacion
                {
                    ...
                    Precio = cot.Precio,
                    PrecioTotal = cot.Total,
                    CotizacionId = NuevoGuidCotiz,
                    ComboId = cot.ComboId,
                    DetalleColorId = cot.DetalleColorId,
                    DetalleTallaId = cot.DetalleTallaId,
                    DetalleColorTallaId = cot.DetalleEstiloId,
                    DetalleEstiloId = cot.DetalleEstiloId
                };
        ...
        private void AgregarlistaToDgv(DataGridView datagrid, List<ListarDetalleCotiz> listaT, List<Producto> listaProducto)
        {
            foreach (var item in listaProducto)
            {
                ...
                {
                    detalleCotiza.DetalleTallaId = int.Parse(comboestilos.SelectedValue.ToString());
                }
        ...
        private void GuardarCotizaciones()
        {
            ...
            foreach (var item in detalleCotiz)
            {
                ...
                else
                {
                    ...
                    {
                        var detalleestiloobten = _tallasyColoresRepository.GetColorTalla((int)item.DetalleColorTallaId);
                        if (detalleestiloobten.Stock < item.Cantidad) { KryptonMessageBox.Show("No hay suficiente en Stock para continuar"); return; }
                    }
                }
       ...
       private void traerRC(Cotizacion coitz)
        ...
        public void LlenarTextBox(ListarProductos filaActual)
        {
            ...
            {
                var listaestilosProductos = _detalleEstiloRepository.GetProductoListaEstilo(productoObtenido.Id);
                CargarComboEstilos(listaestilosProductos);
            }
        }

        private void CargarComboEstilos(List<DetalleEstilo> listaestilosProductos)
        {
            comboestilos.DataSource = listaestilosProductos;
