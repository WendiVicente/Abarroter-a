...
namespace Sistema.Forms.modulo_vales
{
    public partial class NuevoVale : BaseContext
    {
        ...
        private DetalleEstiloRepository _detalleEstilosRepository = null;
        private Cliente _clienteActual = null;

        private int codDetalle = 0, comboidcol = 1,  productidCol=2,  preciocol = 8,cantidadcol = 9, subtotalcol = 10;
        private int coloridCol = 11, tallaidCol = 12, estiloidCol = 14;

        private int colAcciones = 14;
        int colAccionesProd = 31;
        int cuentaclacol = 10;
        int gubernacol = 9;
        int reventacol = 11;
        int minoristacol = 8;
        ...
        {
            _vale = valeRecibido;
            _detalleEstilosRepository = new DetalleEstiloRepository(_context);
        ...
        public void CargarDgvCombos(bool loadNewContext = true)
        {
            ...
            dgvCombos.Columns[14].Visible = true;
        }
        ...
        private void AgregarlistaToDgv(DataGridView datagrid, List<ListarDetalleVales> listaT, List<Producto> listaProducto)
        {
            foreach (var item in listaProducto)
            {
                ...
                detalleVales.Estilo = comboestilos.Text;
                ...
                {
                    detalleVales.DetalleEstiloId = int.Parse(comboestilos.SelectedValue.ToString());
                }
        ...
        private void GuardarVales()
        {
            ...
            foreach (var item in detalleVales)
            {
                ...
                else
                {
                    ...
                    {
                        var detalleestilos = _detalleEstilosRepository.GetDetalleEstilo((int)item.DetalleColorTallaId);
                        if (detalleestilos.Stock < item.Cantidad) { KryptonMessageBox.Show("No hay suficiente en Stock para continuar"); return; }
                        detalleestilos.Stock -= item.Cantidad;
                        _detalleEstilosRepository.Update(detalleestilos);
                    }
                }
        ...
        public List<DetalleVale> GetDatosDetalleVales()
        {
            ...
            foreach (DataGridViewRow item in dgvPedido.Rows)
            {
                ...
                var fila = (ListarDetalleVales)item.DataBoundItem;
                DetalleVale detallevale = new DetalleVale
                {
                    ...
                    precio = fila.Precio,
                    DetalleTallaId = fila.DetalleTallaId,
                    DetalleColorId = fila.DetalleColorId,
                    DetalleColorTallaId = fila.TallayColorId,
                    DetalleEstiloId = int.Parse(item.Cells[estiloidCol].Value.ToString())
                };
        ...
        {
            var fila = dgvPedido.CurrentRow;
            var pedido = (ListarDetalleVales)fila.DataBoundItem;
            if (pedido.ProductoId != 0)
            {
                var productoObtenido = _productosRepository.Get(pedido.ProductoId);
                if (productoObtenido.TieneEscalas)
                {
                    int cantidad = pedido.Cantidad;
                    var TipoPrecio = _tipoPrecioRepository.Get(pedido.ProductoId);
                    var detallePrecios = _tipoPrecioRepository.GetDetallePrecio(TipoPrecio.Id, _vale.TipoPrecio);
                    if (detallePrecios == null) { return; }
        ...
        {           
            ...
            if (productoObtenido.TieneEstilo == true)
            {
                var listadeEstilosProductos = _detalleEstilosRepository.GetProductoListaEstilo(productoObtenido.Id);
                cargarComboEstilos(listadeEstilosProductos);
            }
        }
        ...
        private void CargarSizeAndColorsToCombo()
        {
            ...
            foreach (DataGridViewRow Rows in dgvtallascolores.Rows)
            {
                ...
                else
                {
                    ...
                    detalleVale.Color = Rows.Cells[3].Value.ToString();
                    detalleVale.Talla = Rows.Cells[4].Value.ToString();
